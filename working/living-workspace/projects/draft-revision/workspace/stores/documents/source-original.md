---
kind: source
title: "AI-Driven Photo Editing Methodology (original)"
order: 1
focus: false
...
## 5. Tool Recommendation Strategy

Tool availability is gated by skill level, image type, workflow stage, and editing objective. More tools do not produce better results — for novices they reliably produce worse ones.

Beginners get a small set of high-impact, low-risk controls (crop, straighten, exposure, white balance, highlights, shadows, contrast, vibrance, basic noise reduction, basic sharpening). Intermediate access opens up tone curve, HSL, masks, healing, local adjustments, lens corrections. Advanced access opens luminosity masks, dodge and burn, frequency separation, channel-based adjustments, advanced color grading.
...
**Generate four edit paths per test image**: unguided novice, generic LLM-guided novice, structured-methodology AI-assisted novice, and a professional reference edit with the pro's reasoning captured. The structured system has to beat the generic LLM path, not just the unguided baseline — that's the bar.
...
The system succeeds if it: improves novice outcomes over both unguided editing and generic LLM guidance; teaches rather than just automates; reflects real professional judgment; adapts to image type and user proficiency; maintains workflow state; constrains tool recommendations appropriately; avoids prompt-stuffing through modular injection; produces consistent, debuggable behavior; and shows measurable movement toward professional reference edits while remaining beginner-appropriate.
