...
Not every tool encodes a workflow. The `get_help` tool, for example, is a straightforward retrieval tool — it makes the grounded path the path of least resistance when the LLM is asked a question about how the app works, rather than letting it hallucinate an answer from training data. That's valuable (it's the first level of bounding from the previous section), but it's not workflow enforcement. A future version might add a built-in workflow that compares what's been retrieved against what's needed to fully answer the question, implementing a quality gate that reduces incomplete or hallucinated guidance. But today it's simply a well-tuned retrieval tool.

The research and enrich tools are different. They encode genuine workflow logic — forced step ordering, iteration control, strategy enforcement — that the LLM cannot override or drift away from. That's the distinctive contribution of this principle.
